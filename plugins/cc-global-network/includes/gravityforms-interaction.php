...
function ccgn_application_votes_counts($applicant_id)
{
    ...
    foreach ($votes as $vote) {
        ...
        if (!in_array($voter, $voters)) {
            $did_they = $vote[CCGN_GF_VOTE_APPROVE_MEMBERSHIP_APPLICATION];
...
{
    if (
        $form['title'] == CCGN_GF_CHOOSE_VOUCHERS
...
function ccgn_cleanup_old_records()
{
    ...
    foreach ($entries as $entry) {
        ...
        $status = $entry[CCGN_GF_FINAL_APPROVAL_APPROVE_MEMBERSHIP_APPLICATION];
...
function ccgn_voucher_can_be_chosen_by_applicant(
    $voucher_id,
    $applicant_id
) {
    $result = false;
